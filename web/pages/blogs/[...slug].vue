...
const onShareClick = () => {
  if ('share' in navigator) {
    return navigator.share({ url: shareUrl.value });
  }
  ...
  <ContentDoc v-slot="{ doc }">
    <div class="flex flex-col gap-4">
      <section class="flex flex-col">
        <div class="relative flex flex-col bg-secondary">
          ...
          <div class="content-container z-[1] flex flex-col gap-8 px-4 py-12">
            <div class="flex">
              <NuxtLink :to="{ name: 'blogs', query: { category: doc.category } }"
            ...
            <div class="flex flex-col gap-2">
              ...
              <span class="text-lg text-secondary-900">{{ doc.author }}</span>
            </div>
        ...
        <div class="content-container flex flex-col lg:flex-row justify-between gap-8 bg-white px-4 pt-4 pb-12">
          <main class="prose lg:prose-lg">
            ...
            <ul class="list-none flex !px-0 mt-8 gap-x-2">
              <li v-for="tag in doc.tags" :key="tag">
                <NuxtLink :to="{ name: 'blogs', query: { tags: [tag] } }"
          ...
          <aside
            class="lg:self-start lg:sticky top-1/4 flex lg:flex-col items-center lg:items-end gap-4 lg:pl-8 lg:pr-4 py-4 lg:py-6 border-t-2 lg:border-t-0 lg:border-l-2 border-primary-50">
            <Button rounded outlined @click="onCopy">
              <span>Salin</span>
              <Icon icon="heroicons-solid:clipboard-copy" class="icon" />
            </Button>

            <span class="lg:mr-4">atau</span>

            <Button rounded outlined @click="onShareClick">
              <span>Bagikan</span>
              <Icon icon="heroicons-solid:share" class="icon" />
            </Button>
          </aside>
        </div>
      </section>
      ...
      <template v-else>
        ...
        <Button :to="{ name: 'blogs' }" link class="text-primary">
